...
try:
    ...
    print("grabbed from .env file")
except:
    token = vals['Discord']['Bot Client ID']
    print("dunno what ur talking about man")
...
async def background_timer():
    ...
    channel = client.get_channel(697682410581786714)
    while not client.is_closed():
        ...
        if response:
            if response.status_code == 200:
                ...
                for post in extract_data:
                    ...
                    if post['updated_at'] > (dt.datetime.utcnow() - dt.timedelta(minutes=45)):
                        relevant_data.append(post)
                if len(relevant_data) > 5:
                    num_posts = len(relevant_data)
                    contrib_list = [get_author_name(x['author_id']) for x in relevant_data]
                    ...
                    await channel.send(content="... and more \n See all posts at https://padlet.tk.sg/swift2020submissions")
        ...
        await asyncio.sleep(30)  # asyncio.sleep takes delay in seconds
